  ...
  <div>
    ...
    <el-card >
      <el-row :gutter="30">
        ...
        <el-col :span="3">
          <el-button type="primary" @click="addDialogVisible = true">创建歌单</el-button>
        </el-col>
        <el-col :span="3">
      ...
      <el-table :data="musicList" border stripe>
        ...
        <el-table-column label="歌单名称" v-if="colData[3].istrue" prop="name"></el-table-column>
    ...
    <el-dialog
      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="100px">
        <el-form-item label="歌单Id" prop="listId">
...
export default {
  data () {
    return {
      ...
      colData: [{title: 'listId', istrue: true},
        ...
        {title: '歌单名称', istrue: true},
        {title: '操作', istrue: true}],
      colOptions: ['listId', '图片', '图片URL', '歌单名称', '操作'],
      colSelect: ['listId', '图片', '图片URL', '歌单名称', '操作']
    }
  },
  created () {
    this.getMusicList()
  ...
  methods: {
    // 获取歌单列表
    getMusicList () {
      this.$axios.get('playlist/page', {params: this.queryInfo}).then((response) => {
    ...
    addMusic () {
      this.$refs.addFormRef.validate(valid => {
        ...
        if (!valid) return
        console.log(this.addForm)
        this.$axios.post('playlist', this.addForm).then((response) => {
    ...
    showEditDialog (listId) {
      ...
      this.$axios.get('playlist/id', {params: this.queryPlaylistById}).then((response) => {
    ...
    editPlaylist () {
      this.$refs.editFormRef.validate(valid => {
        if (!valid) return
        this.$axios.put('playlist', this.editForm).then((response) => {
          console.log(response.data)
          if (response.data) {
            this.editDialogVisible = false
            this.getMusicList()
            this.$message.success('更新歌单信息成功！')
          } else { this.$message.error('更新歌单信息失败！') }
        }).catch((response) => {
          this.editDialogVisible = false
          this.$message.error('更新歌单信息失败！')
        })
      })
    },
    async deletePlaylistById (listId) {
      ...
      this.$axios.delete('playlist/', {params: this.queryPlaylistById}).then((response) => {
        this.getMusicList()
